    ...
    for variant in variants:
        ...
        for variant_key, variant_value in variant.attributes.items():
            filter_available_variants[int(variant_key)].append(
                (int(variant_value), variant.future_shipping))

    for attribute in variant_attributes:
        available_variants = filter_available_variants.get(attribute.pk, None)
   #     print("filter_available_variants = " + str(filter_available_variants))
   #     print("variant_attributes = " + str(variant_attributes))
   #     print("available_variants = " + str(available_variants))

        if available_variants:
            future_shipping = {}
            available_variants_ids = []
            for (x, y) in available_variants:
                available_variants_ids.append(x)
                future_shipping[x] = y

   #         print("available_variants_ids = " + str(available_variants_ids))
   #         print("future_shipping = " + str(future_shipping))
                
            # (available_variants_ids, future_shipping) = available_variants
            data['variantAttributes'].append({
                'pk': attribute.pk,
                'name': attribute.name,
                'slug': attribute.slug,
                'values': [
                    {'pk': value.pk, 'name': value.name + (" (*)" if future_shipping[value.pk] else ""),
		        'slug': value.slug}	
                    for value in attribute.values.filter(
                        pk__in=available_variants_ids)]})
